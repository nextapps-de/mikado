...
var F={},E={},ca=document.body,x="ontouchstart"in window,B=!x&&window.PointerEvent&&navigator.maxTouchPoints,ba;G.route=G.prototype.route=function(a,b){E[a]=b;return this};G.dispatch=G.prototype.dispatch=function(a,b,c,d){E[a](b,c,d);return this};var H,I,aa;
if(x||B){var da=function(a,b){b&&(a=b[0]);H=a.clientX;I=a.clientY},ea=function(a){var b=H,c=I;da(a,a.changedTouches);50>Math.abs(H-b)&&50>Math.abs(I-c)&&D.call(this,a,"tap")},fa=function(a){da(a,a.touches)};aa=function(a){l(a,B?"pointerdown":"touchstart",fa,!1);l(a,B?"pointerup":"touchend",ea,!1)}}G.listen=G.prototype.listen=function(a,b){F[a]||(l(1,a,D,b||!0),F[a]=1);return this};G.unlisten=G.prototype.unlisten=function(a,b){F[a]&&(l(0,a,D,b||!0),F[a]=0);return this};k=G.prototype;k.move=function(a,b){if("number"===typeof a){var c=a;a=this.b[c]}else c=this.index(a);0>b&&(b=this.length+b-1);c!==b&&this.shift(a,b-c);return this};
...
k.reconcile=function(a,b,c,d){var e=this.store&&!this.K,f=this.b,g=this.g,h=a.length,m=f.length,n=m>h?m:h,y=0,v=this.key;e&&(this.store=0);for(c||(c=0);c<n;c++){var p=void 0;if(c<h){var r=a[c],q=c>=m,t=void 0,A=void 0,w=void 0;if(!q&&(t=f[c],A=r[v],w=t._k,w===A)){d&&this.update(t,r,b,c);continue}if(d&&(q||!g[A])){q||!this.v?(m++,n=m>h?m:h,this.add(r,b,c)):this.replace(t,r,b,c);continue}for(var z=q=void 0,u=c+1;u<n;u++)if(!q&&u<m&&f[u]._k===A&&(q=u+1),!z&&u<h&&a[u][v]===w&&(z=u+1),q&&z){q>=z?(p=f[q-
1],this.root.insertBefore(p,t||null),d&&this.update(p,r,b,c),q===z?(1<u-c&&this.root.insertBefore(t,f[q]||null),f[c]=f[u],f[u]=t):(W(f,q-1,c),y++)):(r=z-1+y,this.root.insertBefore(t,f[r]||null),W(f,c,(r>m?m:r)-1),y--,c--);p=1;break}}p||(this.remove(c),m--,n=m>h?m:h,c--)}e&&(this.store=a);return this};function W(a,b,c,d){var e=d||a[b];d&&b++;if(b<c)for(;b<c;b++)a[b]=a[b+1];else for(;b>c;b--)a[b]=a[b-1];a[c]=e}
...
function V(a,b,c,d,e){if(!c){var f=S[b.n+(a.cache?"$":"")];if(f)return a.G=f.G,a.F=f.F,a.A=f.A,a.proxy=f.proxy,a.include=f.O,a.u=f.u,f.node}f=document.createElement(b.t||"div");c||(c=0,d="&",X="",a.u=[],f._p=e=[]);var g=b.s,h=b.i,m=b.x,n=b.h,y=b.a,v=b.e,p=b.c,r=b.j,q=a.u.length,t=0,A=0,w="";r&&(w+=";"+r,-1<w.indexOf("self")&&(t=2));b.f&&(X+=";if("+b.f+"){self.hidden=false",t=2);p&&("object"===typeof p?(r=p[1],p=p[0],w+=a.cache?";v="+p+";if(self._c!==v){self._c=v;self.className=v}":";self.className="+
p,r&&(Y(a,p,["_c",q]),A++),t++):f.className=p);if(y||v){var z;y&&(z=Object.keys(y));v&&(p=Object.keys(v),z=z?z.concat(p):p);for(p=0;p<z.length;p++){r=z[p];var u=void 0;y&&"undefined"!==typeof(u=y[r])||(u=v[r],a.listen(r));if("object"===typeof u){var Ia=u[1];u=u[0];w+=a.cache?";v="+u+";var _a=self._a||(self._a={});if(_a['"+r+"']!==v){_a['"+r+"']=v;self.setAttribute('"+r+"',v)}":";self.setAttribute('"+r+"',"+u+")";Ia&&(Y(a,u,["_a",q,r]),A++);t++}else f.setAttribute(r,u)}}g&&("string"===typeof g?f.style.cssText=
g:g.length&&(z=g[1],g=g[0],w+=a.cache?";v="+g+";if(self._cs!==v){self._cs=v;(self._s||(self._s=self.style)).cssText=v}":";self.style.cssText="+g,z&&(Y(a,g,["_cs",q]),A++),t++));if(b["@"]||b.r){a.include||(a.include=[]);var C=b["@"]||b.i;b["@"]||(C.n=b["@"]=a.l+"@"+a.include.length,b.i=null);h=null;w+=";this.include["+a.include.length+"].mount(self).render("+b.r+(b.m?".slice("+(0<=b.m?"0,"+b.m:b.m)+")":"")+",view)";m=X;X="";a.include.push(new G(f,C,Object.assign({},a.N,{store:!1,async:0})));X=m;t++}else if(!h)if(b["+"])h=
R[b["+"]];else if(m){if(n="object"===typeof m)C=m[1],m=m[0];g=document.createTextNode(m);n&&(t&&q++,a.u[q]=d+"|",e[q]=g,Ca(t,a.cache?";v="+m+";if(self._x!==v){self._x=v;self.nodeValue=v}":";self.nodeValue="+m,q,a.cache),C&&(Y(a,m,["_x",q]),A++),t&&q--);f.appendChild(g)}else n&&("object"===typeof n?(C=n[1],n=n[0],w+=a.cache?";v="+n+";if(self._h!==v){self._h=v;self.innerHTML=v}":";self.innerHTML="+n,C&&(Y(a,n,["_h",q]),A++),t++):f.innerHTML=n);t?(a.u[q]=d,e[q]=f,a.F=0,t===A&&(a.A=1),Ca(t,w,q,a.cache)):
w&&(X+=w);if(h)if(h.length)for(A=">",w=0;w<h.length;w++){w&&(A+="+");m=h[w];if(C=m["+"])m=R[C];C=V(a,m,c+w+1,d+A,e);f.appendChild(C)}else{if(C=h["+"])h=R[C];d=V(a,h,c+1,d+">",e);f.appendChild(d)}b.f&&(X+="}else "+(1<t?"self":"p["+q+"]")+".hidden=true");c||(!a.F&&X&&(a.G=Function("p","s","data","index","view",'"use strict";var self,v'+X)),c={G:a.G,F:a.F,u:a.u,node:f},c.O=a.include,c.proxy=a.proxy,c.A=a.A,S[b.n+(a.cache?"$":"")]=c);return f}
