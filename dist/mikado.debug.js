    ...
    for (; x < max_end; x++) {
      ...
      if (x < end_b) {
        ...
        if (!keys[b_x_key] || x >= end_a) {
          ...
          this.add(b_x, view, x, null, true);
          continue;
        }
        var a_x = a[x];
        var a_x_key = a_x["_key"];
        if (a_x_key === b_x_key) {
          a_x["_idx"] = x;
...
Mikado$$module$tmp$mikado.prototype.remove = function(index, count, resize, _skip_indexing) {
  var length = this.length;
  if (index) {
